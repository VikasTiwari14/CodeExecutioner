...
const Pending = () => {
    ...
    useEffect(async() => {
        try{
            const result = await fetch(`/get/pending/question/id/${localStorage.getItem('id')}`,{
                method:"get",
                headers:{
            ...
            <div className='PendingBody'>
                {
                    value?.map((data, index) => {
                        return <div className='PendingCard' title={data?.comment}>
                            ...
                            {data?.solved&&<Button variant='contained' className='viewBtn' onClick={() => handleMark(data?.id)}>View Solution</Button>}
                        </div>
                    })
                }
            </div>
